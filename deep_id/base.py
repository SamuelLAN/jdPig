...
class NN:
    ...
    def save_model_w_b(self):
        ...
        with open(model_path, 'wb') as f:
            pickle.dump([w_all_list, b_all_list, self.mean_x, self.std_x], f, pickle.HIGHEST_PROTOCOL)
    ...
    def restore_model_w_b(self):
        ...
        self.echo('Restoring from %s ...' % model_path)
        with open(model_path, 'rb') as f:
            w_all_list, b_all_list, self.mean_x, self.std_x = pickle.load(f)
      ...
      配置支持的 type:
        ...
        fc_n: 与第 n 层做全连接
            for example:
            {
                'type': 'fc_n',
                'shape': [180, 320],
                'layer_index': 3,       # 第 n 层的 layer_index
                'activate': True,
            }
        dropout: dropout
            for example:
    ...
    def deep_model(self, X, dropout = None):
        ...
        for i, config in enumerate(self.MODEL):
            ...
            if _type == 'conv':
                with tf.name_scope(name):
                    if 'padding' not in config or config['padding'] == 'SAME':
                        padding = 'SAME'
                    else:
                        padding = 'VALID'
    ...
    ''' 2D 卷积 并加上 bias '''
    @staticmethod
    def conv2d_bias(x, W, b, padding='SAME'):
