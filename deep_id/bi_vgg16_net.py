...
class VGG16(base.NN):
    ...
    def init(self):
        ...
        self.mean_x_list = [0 for i in range(self.NUM_PIG)]
        self.std_x_list = [0.1 for i in range(self.NUM_PIG)]
    ...
    def model(self):
        self.__output_list = []
        for i in range(self.NUM_PIG):
            self.__output_list.append( self.deep_model(self.__image_list[i], self.__keep_prob) )


    ''' 重建模型 '''
    def rebuild_model_i(self, i):
        self.__output_list[i] = self.deep_model_rebuild(self.__image_list[i])
    ...
    def get_train_op_list(self):
        ...
        for i in range(self.NUM_PIG):
            with tf.name_scope('optimizer'):
                ...
                self.__train_op_list.append(optimizer.minimize(self.__loss_list[i], global_step=self.__global_step_list[i]))
    ...
    def run(self):
        ...
        for i in range(self.NUM_PIG):
            ...
            for step in range(self.__steps_list[i]):
                if step % self.SHOW_PROGRESS_FREQUENCY == 0:
                    ...
                    self.echo('\r net: %d  step: %d (%d|%.2f%%) / %d|%.2f%% \t\t' % (i + 1, step, self.__iter_per_epoch_list[i], epoch_progress,
                ...
                feed_dict = {self.__image_list[i]: batch_x, self.__label: batch_y, self.__keep_prob: self.KEEP_PROB,
                             self.__size: batch_y.shape[0]}
                run_list = [self.__train_op_list[i], self.__loss_list[i], self.__log_loss_list[i], self.__accuracy_list[i]]
                ...
                if step % self.__iter_per_epoch_list[i] == 0 and step != 0:
                    ...
                    self.mean_x_list[i] = self.__running_mean_list[i]
                    self.std_x_list[i] = self.__running_std_list[i] * (self.BATCH_SIZE / float(self.BATCH_SIZE - 1))
                    ...
                    echo_str = '\n\t net: %d  epoch: %d  train_loss: %.6f  train_log_loss: %.6f  train_accuracy: %.6f  ' \
                               'val_loss: %.6f val_log_loss: %.6f  val_accuracy: %.6f' % (i + 1, epoch, mean_train_loss,
                                        mean_train_log_loss, mean_train_accuracy, val_loss, val_log_loss, val_accuracy)
                    ...
                    if best_val_log_loss > val_log_loss:
                        ...
                        best_mean = self.mean_x_list[i]
                        best_std = self.std_x_list[i]
        ...
        for step in range(self.__steps):
            ...
            _, train_loss, train_log_loss, train_ch_log_loss, train_accuracy = self.sess.run([train_op, self.__loss, self.__ch_log_loss, self.__log_loss, self.__accuracy], feed_dict)
            ...
            if step % self.__iter_per_epoch == 0 and step != 0:
                ...
                # 测试 校验集 的 loss
                mean_val_accuracy, mean_val_loss, mean_val_log_loss, val_ch_log_loss = self.__measure(self.__val_set, 100)
